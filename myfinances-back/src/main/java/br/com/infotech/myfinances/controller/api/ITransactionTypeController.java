...
public interface ITransactionTypeController {
  ...
  @GetMapping
  ResponseEntity<Page<TransactionTypeDto>> findAll(@RequestParam(value = "description", required = false) String description,
                                                   @RequestParam(value = "types", required = false) List<String> types, Pageable pageable);
