...
public interface IUserController {
  ...
  ResponseEntity<Void> changePassword(@Parameter(description = "ID do usuário", required = true) @RequestParam("userId") Long userId,
  ...
  ResponseEntity<Page<UserDto>> findAll(@Parameter(description = "Lista de status para filtro") @RequestParam(value = "statuses",
      required = false) List<UserStatus> statuses, @ParameterObject Pageable pageable);

  @Operation(summary = "Cria um novo usuário", description = "Cria um novo usuário com status ACTIVE.")
  @ApiResponse(responseCode = "200", description = "Usuário criado com sucesso")
  @ApiResponse(responseCode = "400", description = "Dados inválidos.")
  ResponseEntity<UserDto> create(@io.swagger.v3.oas.annotations.parameters.RequestBody(description = "Dados do usuário",
  ...
  ResponseEntity<UserDto> update(@Parameter(description = "ID do usuário", required = true) @PathVariable("id") Long id,
  ...
  @ApiResponse(responseCode = "404", description = "Usuário não encontrado.")
  ResponseEntity<Void> changeStatus(@Parameter(description = "ID do usuário", required = true) @PathVariable("id") Long id,
